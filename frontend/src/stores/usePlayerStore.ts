...
export const usePlayerStore = create<PlayerStore>((set, get) => ({
    ...
    playNext: () => {
        ...
        if(nextIndex < queue.length) {
            ...
            const socket = useChatStore.getState().socket;
            if(socket.auth) {
                socket.emit("update_activity", {userId: socket.auth.userId, activity: `Playing ${nextSong.title} by ${nextSong.artist}`});
            }
    ...
    playPrevious: () => {
        ...
        if(previousIndex >= 0) {
            ...
            const socket = useChatStore.getState().socket;
            if(socket.auth) {
                socket.emit("update_activity", {userId: socket.auth.userId, activity: `Playing ${previousSong.title} by ${previousSong.artist}`});
            }
            set({
